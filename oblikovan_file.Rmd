...
Izkaže se, da v kolikor želimo, da naši testi delujejo tudi za različno porazdeljene markerje, moramo pred izračunom testne statistike in permutiranjem markerja pretvoriti tako, da jim priredimo ranke. S tem na nek način standardiziramo spremenljivki, brez da bi s tem vplivali na vrednost AUC statistike. 

V primeru, ko permutiramo stolpec z bolezmi, se moramo zavedati, da s tem uničimo vpliv markerjev na izid bolezni, torej bi v takšnem primeru markerja mogla biti enakovredna, vendar pa bi imela oba vrednost AUC enako $\frac{1}{2}$.

V nasprotju s permutacijo po stoplcu z boleznimi, v primeru permutiranja po markerjih želimo obdržati njuno medsebojno odvisnot in vpliv na izid bolezni. To naredimo tako, da v vsaki vrstici markerja slučajno zamenjamo ali ne.
...
Odločimo se za $1000$ simulacij za vsak test, za velikosti in za moč.
...
Pogledali smo že, kako naši permutacijski testi delujejo na primeru, kjer sta markerja sicer korelirana, vendar sta enako in normalno porazdeljena. Želimo preveriti, kako se permutacijski testi obašanjo še v drugih, bolj ali manj ekstremnih primerih. V ta namen definiramo različne scenarije, glede na porazdelitve markerjev in glede na velikost vplivov markerjev na izid bolezni.

Podatke generiramo na enak način kot v prejšnjem primeru, le da spreminjamo porazdelitve, velikosti vplivov in velikost začetnega vzorca. V vseh primerih smo za lastnosti testa uporabili $1000$ simulacij. 
...
```{r}
kable(lastnosti1, booktabs=T, caption="Prvotni scenarij") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "responsive"), full_width = F)
```
...
Testa se torej razlikujeta le to tem, ali za testno statistiko vzamemo razliko ali razmerje AUC vrednosti markerjev. Za začetek si poglejmo velikosti za različna testa in scenarije.
...
kable(tabela.out%>%round(2), align = "c", booktabs = T,
      caption = "Velikosti in moči za Test 1 in Test 2") %>%
kable_styling(bootstrap_options = c("striped", "hover", "responsive"), full_width = F) %>%
